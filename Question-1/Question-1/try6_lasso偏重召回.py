...
# 使用逻辑回归分类器（带L1正则化）
model = LogisticRegression(penalty='l1', max_iter=10000)
grid_search = GridSearchCV(estimator=model, param_grid=param_grid, cv=StratifiedKFold(10), n_jobs=-1, verbose=2, scoring='recall')
...
model = LogisticRegression(penalty='l1', max_iter=10000, **best_params)
model.fit(X_train, y_train)

# 调整决策阈值
def adjusted_predict(model, X, threshold=0.3):
    return (model.predict_proba(X)[:, 1] >= threshold).astype(int)

# 在验证集上进行预测并评估
print_progress("在验证集上进行预测并评估")
y_val_pred = adjusted_predict(model, X_val)
print("验证集结果：")
...
y_test_pred = adjusted_predict(model, X_test)
print("测试集结果：")
